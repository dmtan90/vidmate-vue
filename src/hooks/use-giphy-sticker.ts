...
import { GET_STICKER } from "@/api/giphy";
 ...
 * - Manual cache refresh available via refreshTrendSticker()
 * - Cache is page-specific (different pages have separate cache entries)
 */
export const useGiphySticker = defineStore("GiphySticker", (): GiphyStickerReturn => {
	const images = ref([]);
  ...
  const fetchImages = async (url) => {
    ...
    try {
      const response = await GET_STICKER(url);
      if (!response.success) {
  ...
  const loadTrendSticker = async (page = 0) => {
  	// console.log("loadTrendSticker", page);
    const now = Date.now();
