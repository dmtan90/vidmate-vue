...
import { GET } from "@/api/unsplash-image";
...
export interface UseUnsplashImagesReturn {
...
/**
 * Hook for fetching and managing Unsplash images with caching support.
 *
 * Features:
 ...
 */
export const useUnsplashImages = defineStore("unsplashImages", (): UseUnsplashImagesReturn => {
	const images = ref([]);
    ...
    const searchImages = async (query, page = 1) => {
      const url = `/api/unsplash?query=${encodeURIComponent(query)}&page=${page}&per_page=20`;
    ...
    const loadCuratedImages = async (page = 1) => {
      ...
      const url = `/api/unsplash?page=${page}&per_page=20`;
      loading.value = true;
      error.value = null;
