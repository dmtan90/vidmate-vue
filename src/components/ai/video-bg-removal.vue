...
const handleRemoveBackground = async () => {
  try {
    ...
    const blob = await rmbgAI.removeVideoBackground(original, selected.value.name);
    if(!blob){
    ...
    <div class="w-full relative h-full" ref="refContainer" v-loading="loading">
      <template v-if="entry">
        ...
        <div class="bg-transparent-pattern absolute inset-0 overflow-hidden" :style="{ width: position + 'px', height: dimensions.height + 'px' }">
          <video :src="entry.original" class="w-full h-full object-cover object-left-top" />
        </div>
        <VueDraggable
          axis="x"
          :w="10"
      ...
      <template v-else>
        <div class="bg-transparent-pattern">
          <video :src="selected.src" class="w-full h-auto" />
        </div>
      </template>
    </div>
    <template v-if="!entry">
