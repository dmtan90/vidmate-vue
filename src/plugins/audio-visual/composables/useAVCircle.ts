...
import { useCanvasContext, fillCanvasBackground } from './useCanvasContext'
import { Circle, type PropsCircleType } from './useProps'
...
export function useAVCircle<T extends object>(
  ...
  useAudioContext(player, p.fftSize, (data: Uint8Array) => {
    draw(data, ctx, new Circle(props as PropsCircleType))
  })
}
...
export function drawPlaceholder( canvas: Ref<CanvasRenderingContext2D | null>, p: Circle) {
...
function drawProgress(ctx: CanvasRenderingContext2D, p: Circle){
  if (!p.progress) {
    return
  }
  const { currentTime, duration } = p
...
function drawPlaytime(ctx: CanvasRenderingContext2D, p: Circle){
  const { currentTime } = p
