...
export function drawBuffer(data: Uint8Array, canv: Ref<CanvasRenderingContext2D | null>, p: Media) {
  const ctx = resolveUnref(canv)
  if (!ctx) return
  if (p.canvFillColor) ctx.fillStyle = p.canvFillColor
  ctx.clearRect(0, 0, p.canvWidth, p.canvHeight)
...
export function drawVBar(data: Uint8Array, ctx: CanvasRenderingContext2D, p: Media) {
